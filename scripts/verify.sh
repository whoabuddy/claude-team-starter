...
command -v rustc &>/dev/null && ok "rust $(rustc --version | cut -d' ' -f2)" || fail "rust"
command -v clarinet &>/dev/null && ok "clarinet $(clarinet --version 2>/dev/null | cut -d' ' -f2)" || fail "clarinet"
